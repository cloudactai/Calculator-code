...
const MonthlyChecklistTable = () => {
  ...
  const [checklistidCollection, setChecklistidCollection] = useState([]);
  ...
  const handleArchiveChecklist = () => {
    ...
    if (checklistidCollection.length == 0) return toast.error('Please select a checklist to archive');
    let unArchivedData = formsData.filter((data) => !checklistidCollection.includes(data?.id))

    console.log('unArchivedData', unArchivedData)
    setLoaded(true)

    axios.patch(`/update_archive/${getUserSID()}`, {
      ids: checklistidCollection,
      archiveType: "monthly-checklist",
      isArchive: true
    }).then((res) => {
      if (res.data.data.status == "success") {
        toast.success('Archive successfully');
        setFormsData(unArchivedData)
      } else {
    ...
    <Layout title={`Welcome ${response?.username ? response?.username : ""}`}>
      ...
      <div className="panel trans">
        <div className="pHead">
          ...
          <div className="control">
            ...
            {(getCurrentUserFromCookies().role === Roles.ADMIN ||
              getCurrentUserFromCookies().role === Roles.PREPARER
            ) && (
                <Link
        ...
        <div className="pBody pt-0">
          {filter.status === "all" ? (
            <AllMonthlyForms filter={filter} checklistidCollection={checklistidCollection}
              setChecklistidCollection={setChecklistidCollection}
              formsData={formsData}
            ...
            <InprogressMonthlyForms filter={filter}
              checklistidCollection={checklistidCollection}
              setChecklistidCollection={setChecklistidCollection}
              formsData={formsData}
              setFormsData={setFormsData}
            />
