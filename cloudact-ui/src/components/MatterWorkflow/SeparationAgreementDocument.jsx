...
/**
 * Live Separation Agreement document — mirrors
 * "Agreements tool /Separation Agreement (1).docx" paragraph by paragraph,
 * conditionally: a section that would be empty (no children, no shared home,
 * support not applicable) does not render at all, rather than printing a
 * blank clause. Built once, used two ways — live in the browser here, and
 * handed to Flask's /agreement-pdf (xhtml2pdf) for export by serializing
 * this same rendered HTML, so the PDF matches what the lawyer reviewed.
 *
 * One correction from the source docx: its "Child Support" section (¶66-70)
 * reads "...will pay SPOUSAL support..." under a CHILD SUPPORT heading — a
 * copy-paste artifact from the Spousal Support clause below it. That wording
 * is reproduced here as "child support" to match its own heading; nothing
 * about the clause's substance or the rest of the ledger's structure changes.
 *
...
export default function SeparationAgreementDocument({ agreementData }) {
  ...
  return (
    <div className="agreement-doc">
      ...
      <p className="ad-p">
        This Separation Agreement (the &ldquo;Agreement&rdquo;) is entered into between the
        parties named below.
      </p>
      ...
      <h3 className="ad-subheading">Full Disclosure</h3>
      <p className="ad-p">
      ...
      <h3 className="ad-subheading">Purpose</h3>
      <p className="ad-p">
      ...
      <h3 className="ad-subheading">Voluntary Consent</h3>
      <p className="ad-p">
      ...
      {d.hasChildren && (
        <>
          ...
          <p className="ad-p">
            The Parties acknowledge that <Fill value={d.childSupport.payer} placeholder="[payor]" />{" "}
            will pay child support to <Fill value={d.childSupport.recipient} placeholder="[recipient]" />{" "}
            in the amount of $<Fill value={d.childSupport.amount} placeholder="[amount]" />, payable
            monthly, starting on{" "}
            <Fill value={formatDate(d.childSupport.paymentDate)} placeholder="[payment start date]" />.
            This amount is based on the Federal Child Support Guidelines and corresponds to the
            payor&rsquo;s income and the number of children for whom support is provided. The
            Parties agree that: the amount of child support may be reviewed yearly or upon any
            material change in circumstances, including a change in income, parenting
            arrangements, or the children&rsquo;s needs; child support shall continue per
            applicable laws until each child is no longer considered a &ldquo;child of the
            relationship&rdquo; under the Family Law Act or until otherwise agreed in writing or
            ordered by a court; special or extraordinary expenses (also known as section 7
            expenses), such as child care, educational, and medical costs, shall be shared
            proportionately to each Party&rsquo;s income unless otherwise agreed to in writing.
            Nothing in this Agreement prevents either Party from applying to a court for child
            support in accordance with the Family Law Act or the Child Support Guidelines if the
            agreed-upon amount becomes inappropriate in the future.
          </p>
          ...
          {d.parentingTime.party1.include && (
            <>
              ...
              <p className="ad-p">
                ...
                transitions and maintain a consistent routine for the child(ren). Both parties
                further agree to communicate respectfully and promptly with one another regarding
                any necessary changes to the parenting time schedule, and to make reasonable
                efforts to accommodate each other&rsquo;s needs and the best interests of the
                child(ren).
              </p>
            </>
          )}
          ...
          <p className="ad-p">
            ...
            relationship will be held as follows:{" "}
            <Fill value={d.decisionMaking.responsibility} placeholder="[decision-making arrangement]" />.
            Whichever party or parties hold that responsibility shall make decisions regarding the
            child(ren)&rsquo;s health care, education, religious upbringing, and general welfare in
            a manner that prioritizes the best interests of the child(ren), and shall consult with
            the other party in a timely, respectful, and meaningful way before making any
            significant decision affecting the child(ren), except where an emergency requires
            immediate action to protect the health or safety of the child(ren). Each Party shall
            have equal rights to access information from schools, health care providers, and other
            third parties concerning the child(ren). If the parties are unable to reach agreement
            on any matter requiring joint decision-making, they agree to attempt to resolve the
            disagreement through negotiation, mediation, or another mutually acceptable process,
            before seeking a court order.
          </p>
      ...
      {d.spousalSupport.include && (
        <>
          ...
          <p className="ad-p">
            ...
            payable monthly, starting on the{" "}
      ...
      {d.matrimonialHome.hasSharedHome && (
        <>
          ...
          {d.matrimonialHome.sellingOrTransferring === "selling" && (
            <p className="ad-p">
              The Parties agree that the Matrimonial Home shall be sold. The sale proceeds shall
              be shared as follows: <Fill value={d.matrimonialHome.saleProceedsSharing} placeholder="[how proceeds are shared]" />.
              {" "}
      ...
      <p className="ad-p">
        ...
        fair, final, and satisfactory. The Parties agree to the following division of assets and
        property, which reflects either mutual agreement or transfer of ownership. Each Party
        waives any future claim or entitlement to property allocated to the other under this
        Agreement.
      </p>
      ...
      {d.assets.party1.hasKeptAssets && (
        <>
          ...
          <p className="ad-p">
            ...
            <Fill value={p1Name} /> (the &ldquo;Owner&rdquo;). <Fill value={p2Name} /> irrevocably
            waives any and all present or future rights, claims, or entitlements to the following
            property:
          </p>
          {d.assets.party1.items.length > 0 ? (
            <table className="ad-table">
              <thead><tr><th>Asset Type</th><th>Asset Value</th></tr></thead>
              <tbody>
                {d.assets.party1.items.map((item, i) => (
                  <tr key={i}><td>{item.type || "—"}</td><td>{item.value !== "" && item.value != null ? `$${item.value}` : "—"}</td></tr>
                ))}
              </tbody>
            </table>
          ) : (
      ...
      {d.assets.party2.hasKeptAssets && (
        <>
          ...
          <p className="ad-p">
            The Parties agree that the assets listed below are the exclusive property of{" "}
            <Fill value={p2Name} /> (the &ldquo;Owner&rdquo;). <Fill value={p1Name} /> irrevocably
            waives any and all present or future rights, claims, or entitlements to the following
            property:
          </p>
      ...
      {d.equalization.include ? (
        ...
        <p className="ad-p">
          Each Party waives any right to equalization of net family property under the Family Law
          Act and confirms that this waiver is informed, voluntary, and final. Each Party confirms
          that they understand the nature of the rights being waived and agrees that this waiver
          is final.
        </p>
      )}
      ...
      <p className="ad-p">
        ...
        financial liabilities. Each Party shall indemnify and hold the other harmless from any
        claim, demand, or obligation relating to such individual debts and liabilities.
      </p>
      ...
      {d.debts.hasJointDebts && (
        <>
          ...
          {d.debts.items.length > 0 ? (
            <table className="ad-table">
              <thead><tr><th>Type of Debt</th><th>Amount of Debt</th></tr></thead>
              <tbody>
                {d.debts.items.map((item, i) => (
                  <tr key={i}><td>{item.type || "—"}</td><td>{item.amount !== "" && item.amount != null ? `$${item.amount}` : "—"}</td></tr>
                ))}
              </tbody>
            </table>
          ) : (
