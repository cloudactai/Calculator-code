"""Remove the two blocks F1's flatten printed twice (p2's language options, p4's header).
...
Run: python3 repair_f1_duplicates.py [--apply]
"""
import argparse
import json
import os
...
# Bands whose *text* is the duplicate copy.
TEXT_BANDS = [
...
DROP_FIELDS = {1750498416032, 1750498416033, 1750498416034}
...
def main():
    ...
    before = {}
    for page_number, phrases in KEEP_TEXT.items():
    ...
    keep_ids = {int(f["id"]) for f in mapping["staticFields"]} - DROP_FIELDS
    for page_number, band in TEXT_BANDS + PAINT:
    ...
    if apply_it:
        ...
        kept = [f for f in mapping["staticFields"] if int(f["id"]) not in DROP_FIELDS]
        ...
        doc.close()
        print("\ndry run — would drop %d field(s)" % len(DROP_FIELDS))
