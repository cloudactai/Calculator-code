"""Add the prefill binds to the promoted Nova Scotia templates.
...
The vocabulary and the reasoning for it are in `ns_binds.py` -- in short, the
party role is read from the word Nova Scotia prints to the **right** of each
        ...
        for number in range(1, document.page_count + 1):
            ...
            for field in fields:
                if field["page"] != number or field["type"] == "CheckBox":
                    continue
                box = rect_of(field)
                bind = ns_binds.bind_for_role(words_beside(words, box, "right"))
                if not bind:
                    bind = ns_binds.bind_for_caption(
...
def main():
    ...
    for name in sorted(os.listdir(EXPORT)):
        if not name.startswith("NS") or not name.endswith(".json"):
            continue
    ...
    print("\n%s %d binds across %d NS templates"
