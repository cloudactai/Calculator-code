...
all.** 70B, 70B*, 70D, 70E and 70A's item 35 now each get the rest of their
page, 12 to 31 writing lines, with the date/signature/service block that
followed spilled behind them. 70F and 70G are sized to fit their own sheet
instead — see **A page is worth more than seven lines**, below.
...
## Eighth pass: four sheets that carried nothing

Two unrelated defects, both of which cost a sheet of paper and neither of which
a field table would have flagged, because in both cases the extra page was
structurally valid and simply empty.

### Form 70BB shipped two pages of nothing

`PEISC_70BB.pdf` was six pages. Page 6 was completely blank — no text, no
field, no drawing. Page 5 held one ruled cell at (107.6, 115.5)–(514, 139)
with a field in it and no caption, heading or prompt anywhere on the sheet.
Both came straight from the LibreOffice render, not from any pass here: the
staged `_incoming_pei/PEISC_70BB_source.pdf` has the same six pages and the
same four rules on its page 5.

The cell is the tail of page 4's table, and reading the source `.docx` says
exactly which part. The last table has ten rows; the tenth is the signature
row, and it holds eight paragraphs — three blank, the `Date`/`Signature` rule,
its caption, then three more blank. Six of those fit in the 69.8pt the row gets
on page 4; the last two paginated overleaf and took the row's own left, right
and bottom rules with them. So the cell is not a spilled answer band and it is
not a row that lost its heading — **it is two empty trailing paragraphs of the
signature row, and there is nothing it could be moved back to page 4 to hold.**
Every one of the ten rows already prints there in full, item 19's answer band
included. The field over it was auto-generated by the cell detector, has no
bind and answers no prompt.

Both pages dropped, and the field with page 5. Pages 1–4 render pixel-identical
before and after; the drawing census falls by exactly the four rules that were
on page 5, and the extracted text is token-for-token unchanged.

> This does not touch the signature gap. Page 4's `Signature` rule has no
> field, the same batch-wide decision recorded in **Boxes on signature lines**;
> re-purposing an orphan into it would have overturned that on no evidence.

### A page is worth more than seven lines

The sixth pass gives a narrative prompt `None` for its box height, meaning
"take the rest of the page", and spills whatever followed onto a continuation.
On five of the seven pleadings that is the right trade. On **70F** and **70G**
it was not: both are one-page forms whose prompt sits low and whose entire tail
is the date, name, address, phone and `TO:` block, so each bought a second
sheet that carried nothing else.

They now take an explicit height — the clear paper above that tail, 136.40pt
and 194.15pt measured, rounded down to 136 and 194 — and both are one page
again. The cost is 17 → 10 and 21 → 15 writing lines, which still answers a
numbered-paragraph prompt. The other five keep `None`: their prompts sit higher
and their tails are longer, so sizing them would cost far more than the sheet
is worth.

Re-running the pass on a form it has already spaced is not idempotent — it cuts
a second time. `apply` refuses a form that already has an `answer_space_shifts`
entry, so redoing one means reverting its `.pdf` and `.json` to the pre-pass
commit, deleting its entry from `answer_space_shifts.json`, and running with
`--only`.

```
python3 pei_answer_space.py --check --only PEISC_70F
